...
use Admin\Controller\Concerns\UsersControllerFiltersTrait;
use Admin\Controller\Concerns\UsersControllerSubscriptionTrait;

class UsersController extends BaseController
{
    use UsersControllerFiltersTrait;
    use UsersControllerSubscriptionTrait;
    ...
    public function index(): void
    {
        ...
        $users = $this->enrichUsersWithSubscriptionContext($usersModel->allWithGroup(), $currentHierarchyLevel);

        $planOptions = array_values(array_filter(
            $this->subscriptionService()->adminPlans(),
            static fn (array $plan): bool => (int) ($plan['status'] ?? 0) === 1
        ));
        $featureCatalog = $this->subscriptionService()->featureCatalog();
        $skipDefaultFilters = (int) $this->request->get('skip_default_filters', 0) === 1;
        $listFilters = $this->usersListFilter()->normalize((array) $this->request->get);
        $usingSavedDefaultFilters = false;
        $hasSavedDefaultFilters = $this->hasSavedUsersListFilters($currentUserId);

        if (!$skipDefaultFilters && !$this->usersListFilter()->hasInput((array) $this->request->get)) {
        ...
        $users = $this->usersListFilter()->apply($users, $listFilters);
        ...
        $this->render('users/index', [
            ...
            'list_filters_query' => $this->usersListFilter()->buildQuery($listFilters, $skipDefaultFilters),
    ...
    public function store(): void
    {
        ...
        $usersModel = $this->loader->model('users');
        if ($usersModel->existsByEmail($email)) {
