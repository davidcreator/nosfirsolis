    ...
    public function clearCache(): void
    {
        ...
        try {
            ...
            flash('error', $this->t(
                ...
                'Nao foi possivel limpar o cache do sistema.'
            ));
            $this->redirectToRoute('operations/index');
        }
        ...
        $observability->log(
            'info',
            'operations',
            $this->t('operations.log_cache_cleared', 'Limpeza de cache executada no painel admin.'),
            [
                'removed_files' => (int) ($result['files'] ?? 0),
                'removed_directories' => (int) ($result['directories'] ?? 0),
                'opcache_reset' => (bool) ($result['opcache_reset'] ?? false),
            ],
        ...
        flash('success', $this->t(
            'operations.flash_cache_cleared',
            'Cache limpo. Arquivos removidos: {files}. Pastas removidas: {directories}.',
            [
                'files' => (int) ($result['files'] ?? 0),
                'directories' => (int) ($result['directories'] ?? 0),
            ]
        ));
        $this->redirectToRoute('operations/index');
    }
